    ...
    try {
      ...
      if (existente != null) {
        ...
        final inserted = await _supabase
            ...
            .single();
        resultado = inserted;
        print('✅ Nueva entrada del diario guardada');
      }
  ...
  Future<int> getDiasConsecutivos() async {
    ...
    try {
      ...
      for (final entrada in entradas) {
        ...
        final fechaEntradaDate = DateTime(fechaEntrada.year, fechaEntrada.month, fechaEntrada.day);

        // Verificar si la fecha de la entrada coincide con la fecha esperada
        if (fechaEntradaDate.isAtSameMomentAs(fechaEsperada)) {
          diasConsecutivos++;
          fechaEsperada = fechaEsperada.subtract(const Duration(days: 1));
        } else if (fechaEntradaDate.isBefore(fechaEsperada)) {
