...
class _BibliotecaScreenState extends State<BibliotecaScreen> {
  ...
  void _aplicarFiltros() async {
    ...
    if (_tab == 'Favoritos') {
      ...
      print('🔍 [FILTROS] Favoritos: ${favoritos.map((f) => f.id).toList()}');
    } else {
  ...
  Future<void> _buscarConIA(String consulta) async {
    try {
      ...
      // Buscar códigos usando OpenAI service
      ...
      if (mounted) {
        if (sugerencias.isNotEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('✅ ${sugerencias.length} códigos sugeridos encontrados'),
              backgroundColor: const Color(0xFF4CAF50),
              duration: const Duration(seconds: 4),
            ),
          );
          // Recargar datos después de la búsqueda
          await _loadData();
        } else {
