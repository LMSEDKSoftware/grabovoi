...
class _EvolucionScreenState extends State<EvolucionScreen>
  ...
  void initState() {
    ...
    // Restricción de Evolución eliminada - todos los usuarios pueden acceder
    ...
    try {
      ...
      final response = await supabase
          ...
          .inFilter('action_type',
              ['sesionPilotaje', 'codigoRepetido', 'pilotajeCompartido']);
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      final sessionHistory =
          await _progressService.getSessionHistory(limit: 500);
      final totalMinutes = sessionHistory.fold<int>(
          0, (acc, s) => acc + ((s['duration_minutes'] as int?) ?? 0));
      ...
      final activeChallenges = userChallenges
          .where((c) => c.status == ChallengeStatus.enProgreso)
          .toList();
      final completedChallenges = userChallenges
          .where((c) => c.status == ChallengeStatus.completado)
          .toList();
  ...
  Widget _buildEnergyLevelCard() {
    final nivel =
        userProgress?['nivel'] ?? userProgress?['nivel_energetico'] ?? 1;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildProgressRow('Códigos Explorados',
              '${_cachedExploredCodesCount ?? 0}', Icons.explore),
        ],
      ),
    );
  }
  ...
  Widget _buildStatsCard() {
    ...
    if (dias > 0) {
      ...
      tiempoStr =
          horasRestantes > 0 ? '${dias}d ${horasRestantes}h' : '${dias}d';
    } else if (horas > 0) {
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              _buildStatItemWithFlame(
                  'Racha', '$racha días', Icons.local_fire_department),
              _buildStatItem('Desafíos', '${completedChallenges.length}',
                  Icons.emoji_events),
  ...
  Widget _buildStatItemWithFlame(String label, String value, IconData icon) {
    return Column(
      children: [
        AnimatedBuilder(
