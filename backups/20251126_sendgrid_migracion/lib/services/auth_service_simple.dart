...
class AuthServiceSimple {
  ...
  Future<AuthResponse> signUp({
    ...
    try {
      ...
      User? createdUser;
      bool userCreatedDespiteError = false;
      ...
      try {
        ...
        // Si hay un error, verificar si el usuario se creó de todas formas
        final errorString = signUpError.toString().toLowerCase();
        final isEmailError = errorString.contains('sending confirmation email') || 
        ...
        // Verificar si es un error relacionado con email (SMTP o rate limiting)
        final isRateLimitError = errorString.contains('rate limit') || 
            errorString.contains('429') || 
            errorString.contains('over_email_send_rate_limit');
        
        if (isEmailError || isRateLimitError) {
          print('⚠️ Error de email/rate limit detectado - verificando si el usuario se creó...');
          ...
          try {
            ...
            if (userCheck != null) {
              if (isRateLimitError) {
                print('✅ Usuario encontrado en tabla users - registro exitoso a pesar del rate limit de email');
              } else {
                print('✅ Usuario encontrado en tabla users - registro exitoso a pesar del error de email');
              }
              userCreatedDespiteError = true;
              // El usuario se creó, continuar con el flujo normal
              // No necesitamos crear el User object, solo procesar el usuario de la tabla
            } else {
              if (isRateLimitError) {
                print('⚠️ Usuario no encontrado en tabla users - rate limit activo, registro no completado');
              } else {
                print('⚠️ Usuario no encontrado en tabla users - registro falló');
              }
              rethrow;
            }
          } catch (checkError) {
            print('⚠️ No se pudo verificar si el usuario existe: $checkError');
            rethrow;
          }
        } else {
          // Si no es un error de email, lanzar el error normalmente
          rethrow;
        }
      }

      // Procesar usuario si se creó exitosamente (ya sea normalmente o a pesar del error de email)
      if (createdUser != null || userCreatedDespiteError) {
        if (userCreatedDespiteError) {
          print('✅ Usuario registrado a pesar del error de email');
        } else {
        ...
        // Si el usuario se creó a pesar del error, retornar respuesta exitosa
        if (userCreatedDespiteError) {
      ...
      // Verificar si el error es solo sobre envío de email
      ...
      if (isEmailError) {
        print('⚠️ Error relacionado con envío de email - verificando si el usuario se creó...');
        
        // Verificar si el usuario se creó exitosamente a pesar del error del email
        try {
          ...
          if (userCheck != null) {
            print('✅ Usuario creado exitosamente - solo falló el envío del email. Continuando sin error.');
            // El usuario se creó, así que el registro fue exitoso
            // Cargar el usuario y continuar normalmente sin mostrar error
            _currentUser = app_models.User.fromJson(userCheck);
            await _saveUserToLocal(_currentUser!);
            ...
            // Retornar respuesta exitosa (sin lanzar error)
            return AuthResponse(
              user: null,
              session: null,
            );
        ...
        // Si el usuario no se creó, lanzar el error
        print('⚠️ Error relacionado con envío de email y registro falló - probablemente falta SMTP en Supabase');
