...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _signUp() async {
    ...
    try {
      ...
      // Si el error es sobre email o rate limit, verificar si el usuario se creó exitosamente
      final isEmailOrRateLimitError = errorString.contains('Error al enviar email de confirmación') ||
          errorString.contains('sending confirmation email') ||
          errorString.contains('Error sending email') ||
          errorString.contains('rate limit') ||
          errorString.contains('429') ||
          errorString.contains('over_email_send_rate_limit') ||
          errorString.contains('demasiados correos');
      
      if (isEmailOrRateLimitError) {
        // Verificar si el usuario se creó consultando el servicio de autenticación
        try {
          final currentUser = _authService.currentUser;
          if (currentUser != null && currentUser.email == _emailController.text.trim()) {
            // El usuario se creó exitosamente, solo falló el email/rate limit
            // No mostrar error, continuar con el flujo normal
            print('✅ Usuario creado exitosamente - solo falló el envío del email/rate limit. Continuando sin error.');
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
