...
class AuthServiceSimple {
  ...
  // Solicitar OTP para recuperación de contraseña (flujo OTP personalizado)
  Future<void> resetPassword({required String email}) async {
    try {
      print('📧 Solicitando OTP para recuperación de contraseña: $email');
      print('   Usando flujo OTP personalizado (send-otp)');
      ...
      if (res.status != 200) {
        ...
        throw Exception('Error ${res.status}: $errorMsg');
      }
      
      if (data == null || (data is Map && data['ok'] != true)) {
        final errorMsg = data is Map ? (data['error'] ?? 'No se pudo enviar el OTP') : 'Respuesta inválida';
        print('❌ Error en respuesta: $errorMsg');
        throw Exception(errorMsg);
      }
      
      print('✅ OTP enviado correctamente');
      print('   Revisa tu email para obtener el código OTP');
    } catch (e, stackTrace) {
  ...
  // Verificar OTP y actualizar contraseña usando admin.updateUserById (versión simple)
  Future<void> verifyOTPAndResetPassword({
    ...
    try {
      print('🔐 Verificando OTP y actualizando contraseña...');
      print('   Email: $email');
      print('   Token OTP: ${token.substring(0, 3)}...');
      print('   Nueva contraseña: ${newPassword.length} caracteres');
      
      // Llamar a la Edge Function que verifica OTP y actualiza contraseña
      ...
      print('✅ OTP verificado y contraseña actualizada exitosamente');
      
    } catch (e, stackTrace) {
      print('❌ Error en verificación OTP o actualización de contraseña: $e');
      print('📚 Stack trace: $stackTrace');
      rethrow;
    }
  }
