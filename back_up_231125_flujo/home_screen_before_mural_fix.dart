...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    ...
    // Verificar WelcomeModal cuando cambien las dependencias (puede ser después del tour)
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (mounted) {
        await _checkWelcomeModal();
      }
  ...
  void didUpdateWidget(HomeScreen oldWidget) {
    ...
    // Verificar WelcomeModal cuando el widget se actualice (puede ser después del tour)
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (mounted) {
        await _checkWelcomeModal();
      }
  ...
  /// Solo se muestra después de que el tour termine
  Future<void> _checkWelcomeModal() async {
    ...
    print('🔍 Verificando WelcomeModal - tourCompleted: $tourCompleted, welcomeModalShown: $welcomeModalShown');

    // Mostrar solo si:
    // 1. El tour ya terminó (tourCompleted == true)
    // 2. El modal de bienvenida nunca se mostró
    if (tourCompleted && !welcomeModalShown && mounted) {
